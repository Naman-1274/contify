...
"""
    ...
    Line 2: Benefit or feature, use USP/fabric/attribute/Discounts%, written as a natural sentence that connects to the headline.
    ...
    if data['category'] == "Concise Content":
        return f"""Write a concise festive ad, 2–3 **natural flowing lines** (80–140 characters total), like something a real person would say to a friend, not a robot. The ad should feel like a compressed, meaningful story that could actually persuade.

    Structure:
    Line 1: An inviting headline or occasion (festival/sale/product) — must stand alone, stated conversationally.
    Line 2: The core product benefit—use USP, fabric, comfort or stand-out attribute. Tie this to the context set above, not just a generic product line.
    Line 3: If a festival or gifting theme is present, weave it in emotionally and naturally—otherwise, use gentle urgency (timing/offer/season). End with a friendly invitation to act (e.g., Shop Now, Treat them, Don’t miss out).
    ...
    **Write as if for a premium brand, but keep the flow and word choices as humans actually speak. Never use: {', '.join(BANNED_WORDS)}.**
    ...
    EXAMPLE 1 (compressed but natural, not a list):
    ...
    **Return ONLY the 2–3 ad lines, each on its own line, no bullets or lists. They must read as naturally and smoothly as the examples.**
    ...
    • Follow with a benefit or shopping context using the product, fabric, USP, Discounts%, festival, or timing fields—but always connected with simple, human phrasing.
    ...
    Line 2: The brand, collection, or product name—one clean line, Discounts%, no extra description.
    Line 3: Benefit, craft, or heritage line—show the special quality or value of the product, using fabric, benefits, or craftsmanship.
    ...
    Do not exceed 200–250 characters in total. Each line must build on the last—no abrupt topic jumps or fragments. All phrases must sound like natural, brand-quality daily English. DO NOT use these words anywhere: {', '.join(BANNED_WORDS)}.

    EXAMPLE (for flow and tone):
